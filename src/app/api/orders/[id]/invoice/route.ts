      ...
      <div class="max-w-4xl mx-auto bg-white border border-gray-200 rounded-xl shadow-sm p-8 sm:p-12">
        ...
        <div class="no-print mb-8 flex justify-between items-center bg-gray-100 p-4 rounded-lg">
          ...
          <div class="flex gap-2">
            <button onclick="window.print()" class="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-black text-sm font-bold rounded-lg transition-colors">
              Print Invoice
            </button>
            <button onclick="window.close()" class="px-4 py-2 bg-gray-300 hover:bg-gray-400 text-gray-800 text-sm font-semibold rounded-lg transition-colors">
              Close Window
            </button>
          </div>
        </div>
        ...
        <div class="flex justify-between items-start border-b border-gray-200 pb-8 mb-8">
          <div>
            ...
            <div class="text-xs text-gray-400 mt-4 leading-relaxed">
              Gifwoods Inc.<br>
              123 Woodworkers Lane, Craftsville<br>
              Karnataka, India - 560001<br>
              support@gifwoods.in
            </div>
          </div>
          <div class="text-right">
            <span class="inline-block px-3 py-1 text-xs font-semibold uppercase tracking-wider rounded-full bg-green-100 text-green-800">
              ${typedOrder.status}
            </span>
                ...
                <td class="py-4">
                  ...
                  ${item.customization ? `
                    <div class="text-xs text-yellow-600 mt-1">
                      Custom: ${Object.entries(item.customization).map(([k, v]) => `${k}: ${v}`).join(", ")}
                    </div>
                  ` : ""}
                </td>
        ...
        <div class="flex justify-end">
          <div class="w-64 space-y-3 text-sm text-gray-600">
            <div class="flex justify-between">
              <span>Subtotal</span>
              <span class="font-medium text-gray-800">${formatPrice(typedOrder.subtotal)}</span>
            </div>
            <div class="flex justify-between">
              <span>Shipping</span>
              <span class="font-medium text-gray-800">${typedOrder.shipping_cost === 0 ? "Free" : formatPrice(typedOrder.shipping_cost)}</span>
            </div>
